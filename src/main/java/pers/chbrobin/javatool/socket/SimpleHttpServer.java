...
public class SimpleHttpServer implements Runnable {
    ...
    public SimpleHttpServer(String serverHtml) {
            ...
            try {
                ...
                if (client != null) {
                    ...
                    try {
                        ...
                        //如果是POST请求，则有请求体  
                        if (resource.startsWith("/proxy") && "POST".equalsIgnoreCase(method)) {
                            //注，这里只是简单的处理表单提交的参数，而对于上传文件这里是不能这样处理的，  
                            //因为上传的文件时消息体不只是一行，会有多行消息体
                            String query = readLine(is, contentLength);
                            ...
                            String result = HttpRequestUtil.getRequestByPost(host + "/" + action, param, "GBK");
                            ...
                            closeSocket(client);
                            System.out.println("query " + query + " map " + map);
                            System.out.println();
                        }
    ...
    void fileDownload(String fileName, Socket socket) {
        try {
            PrintStream out = new PrintStream(socket.getOutputStream(), true);
            File fileToSend = new File(SimpleHttpServer.class.getClassLoader().getResource("jquery.js").toURI().toString());
            if (fileToSend.exists() && !fileToSend.isDirectory()) {
                out.println("HTTP/1.0 200 OK");//返回应答消息,并结束应答  
     ...
     * @param socket 客户端 socket.
     */
    void fileShow(String fileName, Socket socket) {
        try {
            PrintStream out = new PrintStream(socket.getOutputStream(), true);
            File fileToSend = new File(SimpleHttpServer.class.getClassLoader().getResource("jquery.js").toURI().toString().replaceAll("file:/",""));
            if (fileToSend.exists() && !fileToSend.isDirectory()) {
                // 用 writer 对客户端 socket 输出一段 HTML 代码
                out.println("HTTP/1.0 200 OK");//返回应答消息,并结束应答
                out.println("Content-Type:text/html;charset=" + encoding);
                ...
                String fileContent = FileUtil.readFileString(new FileInputStream(fileToSend));
